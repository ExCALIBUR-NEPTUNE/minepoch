...
MODULE deltaf_loader2

  USE shared_data
  
  IMPLICIT NONE

  PUBLIC :: params_local, params_local_all, delstaf_load

CONTAINS

  ! Find local particle temperature: at the moment, just copied and
  ! pasted from particle_temperature.F90
  SUBROUTINE params_local_all(current, temperature, drift, density, temp_local, drift_local, dens_local)

    TYPE(particle), POINTER, INTENT(IN) :: current
    REAL(num), DIMENSION(1-ng:,1-ng:,1-ng:,:), INTENT(IN) :: temperature, drift
    REAL(num), DIMENSION(1-ng:,1-ng:,1-ng:), INTENT(IN) :: density
    REAL(num), INTENT(INOUT), dimension(3) :: temp_local, drift_local
    ...
    DO ix = sf_min, sf_max
       ...
       DO idir = 1,3
          temp_local(idir) = temp_local(idir) &
               + gf * temperature(cell_x+ix, cell_y+iy, cell_z+iz, idir)
          drift_local(idir) = drift_local(idir) &
               + gf * drift(cell_x+ix, cell_y+iy, cell_z+iz, idir)
       END DO
       dens_local = dens_local &
               + gf * density(cell_x+ix, cell_y+iy, cell_z+iz)
    END DO
    END DO
    END DO

  END SUBROUTINE params_local_all
...
#include "particle_to_grid.inc"
  ...
  SUBROUTINE delstaf_load(ispecies, species_temp, species_drift)
   ...
   ! f0 calculation: mainly, we need to calculate the phase space volumes.
    ...
    DO WHILE(ipart < partlist%count)
      ...
      f0_exponent = ((current%part_p(1) - driftx)**2 / Tx ) / two_kb_mass
